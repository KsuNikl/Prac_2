...
func decrypt(p, x int, cipherData []byte, logFile *os.File) []byte {
    ...
    fmt.Fprintln(logFile, "Дешифрование:")
...
func main() {
    ...
    // Имитация атаки "человек посередине"
    _, _, xAttack, _ := keysGenerator()
    decryptedData := decrypt(p, xAttack, cipherData, outFile)
    decFile.Write(decryptedData)

    outFile.WriteString("\nПроверка: ")
    if string(plainData) == string(decryptedData) {
        outFile.WriteString("Успешно! Данные совпадают.\n")
    } else {
        outFile.WriteString("Ошибка! Данные не совпадают (ожидаемо при атаке).\n")
    }
